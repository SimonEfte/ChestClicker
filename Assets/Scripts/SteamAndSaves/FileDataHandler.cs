...
public class FileDataHandler 
{
    ...
    public GameDataJSON Load(string profileId, bool allowRestoreFromBackup = true)
    {
        // Use Path.Combine to account for different OS's having different path separators
        string fullPath = Path.Combine(dataDirPath, profileId, dataFileName); //(NEW) ProfileId is new
        GameDataJSON loadedData = null;
        if (File.Exists(fullPath))
        {
            try
            {
                ...
                loadedData = JsonUtility.FromJson<GameDataJSON>(dataToLoad);
            }
    ...
    public void Save(GameDataJSON data, string profileId)//(New) profileId is new
    {
        // Use Path.Combine to account for different OS's having different path separators
        ...
        try
        {
            ...
            if (DataPersistenceManager.saveIncrement == 2)//(New)
            {
                GameDataJSON verifiedGameData = Load(profileId); //(New)
    ...
    public static bool hadToBackup;//(New)
